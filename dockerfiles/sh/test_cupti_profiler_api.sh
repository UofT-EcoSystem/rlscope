...
main() {
  ...
  make
  cp -r /usr/local/cuda/extras/CUPTI/samples /home/${RLSCOPE_USER}/CUPTI-samples
  chown -R ${RLSCOPE_USER}:${RLSCOPE_USER} /home/${RLSCOPE_USER}/CUPTI-samples
  # Fails with error 35 during docker build...
  # This happens when we haven't set "NVreg_RestrictProfilingToAdminUsers=0"
  set +e
  (
    set -e
    cd /home/${RLSCOPE_USER}/CUPTI-samples/userrange_profiling
    local cmd="LD_LIBRARY_PATH=/usr/local/cuda/extras/CUPTI/lib64 ./userRangeSample"
    echo "> CMD:"
    echo "  $ $cmd"
    echo "  PWD=$PWD"
    echo "  USER=${RLSCOPE_USER}"
    runuser ${RLSCOPE_USER} -c "$cmd"
  )
  local ret=$?
  set -e
