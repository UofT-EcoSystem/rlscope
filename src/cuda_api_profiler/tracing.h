...
//  // EventCollector::IsEnaled() returns false.
//  ScopedRegion(EventCategory category, const std::string& name)
//      : collector_(GetEventCollector(category)) {
//    if (collector_) {
...
class TraceCollector {
 public:
  ...
  virtual std::unique_ptr<Handle> CreateAnnotationHandle(
      const std::string& name_part1, const std::string& name_part2) const = 0;
  virtual std::unique_ptr<Handle> CreateActivityHandle(
      const std::string& name_part1, const std::string& name_part2,
 ...
 protected:
  static std::string ConcatenateNames(const std::string& first, const std::string& second);
...
class ScopedAnnotation {
 public:
  explicit ScopedAnnotation(const std::string& name)
      : ScopedAnnotation(name, std::string()) {}
  ...
  // label std::string is only done if tracing is enabled.
  ScopedAnnotation(const std::string& name_part1, const std::string& name_part2)
      : handle_([&] {
...
class ScopedActivity {
 public:
  explicit ScopedActivity(const std::string& name, bool is_expensive = true)
      : ScopedActivity(name, std::string(), is_expensive) {}

  // If tracing is enabled, set up an activity with a label of
  // "<name_part1>:<name_part2>".  This can be cheaper than the
  // single-argument constructor because the concatenation of the
  // label std::string is only done if tracing is enabled.
  ScopedActivity(const std::string& name_part1, const std::string& name_part2,
