...
#define CONFIG_TRACE_STATS_C_API
...
#ifdef CONFIG_TRACE_STATS_C_API
TF_CAPI_EXPORT extern void TF_PreallocateTracer(
    TF_Session* session,
    int step,
    // Output status
    TF_Status* status);

// Get all the data collected from the profiler over the past few traced steps.
//
// Data is returned as a serialized TraceDataProto proto.
TF_CAPI_EXPORT extern TF_Buffer* TF_GetTraceData(TF_Session* session);
TF_CAPI_EXPORT extern bool TF_IsTracerSet(TF_Session* session);
TF_CAPI_EXPORT void TF_DumpTraceDataAsync(
    TF_Session* session,
    const char* dump_path,
    const char* process_name,
    const char* phase,
    const char* machine_name);
TF_CAPI_EXPORT extern void TF_AwaitTraceDataDumps();
#endif
