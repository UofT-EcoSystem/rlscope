...
bool GetGraphDef(TF_Graph* graph, rlscope::GraphDef* graph_def) {
  ...
  MY_EXPECT_EQ(TF_OK, TF_GetCode(s), s);
...
CSession::CSession(TF_Session* session, bool close_session) : session_(session), close_session_(close_session) {}
...
void CSession::CloseAndDelete(TF_Status* s) {
  DeleteInputValues();
  ResetOutputValues();
  if (session_ != nullptr && close_session_) {
